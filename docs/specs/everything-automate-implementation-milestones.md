...
- `$brainstorming`, `$planning`, `$execute`, `$qa` skill
- `$issue-capture`, `$issue-pick` support skill
- planning, execute, QA review용 Codex agent prompt
- state/progress/runtime helper 초안
- global setup, doctor, backup, manifest 흐름

아직 조심해야 할 점:

- 문서 일부는 예전 Ralph loop 연구나 provider-neutral 설계를 기준으로 쓰였다.
- 현재 source of truth는 Codex template, skill, installer 쪽에 더 가깝다.
- hidden runtime은 아직 메인 UX보다 앞에 나오면 안 된다.
- `$execute`와 `$qa`의 handoff, retry, evidence 기록은 계속 다듬어야 한다.
...
현재 상태: usable draft가 있다. 큰 방향은 잡혔고, 실제 사용에서 질문 흐름과 output brief를 더 다듬으면 된다.
...
현재 상태: usable draft가 있다. planning agent roster와 handoff 생각은 잡혔지만, plan artifact와 실제 `$execute` 입력의 연결은 계속 검증해야 한다.
...
현재 상태: usable draft가 있다. TC-first 흐름, checklist helper, QA handoff 방향이 들어왔고, 지금은 실제 작업에서 부족한 edge를 찾는 단계다.
...
현재 상태: usable draft가 있다. `code-reviewer`와 `harness-reviewer` lane이 생겼고, `$execute` 뒤에 자동 또는 명시적으로 들어갈 수 있다.
...
현재 상태: 진행 중이다. `ea_state.py`, `ea_progress.py`, `ea_codex.py`, execute checklist helper, QA handoff helper가 있다. 이 단계의 핵심은 helper가 workflow의 주인이 되지 않게 하고, 필요한 기록과 검증만 조용히 받치게 만드는 것이다.
...
현재 상태: 일부 완료다. `setup`과 `doctor`는 작동하고, global install은 Codex AGENTS, agents, workflow skills, support skills를 설치한다. 남은 일은 README와 설치 안내가 항상 실제 설치 결과와 맞게 유지되도록 정리하는 것이다.
...
1. 루트 README, docs index, Codex install 문서가 실제 설치 결과와 맞는지 계속 맞춘다.
2. `$execute`에서 AC/TC/checklist/evidence 흐름이 실제 작업에 충분한지 검증한다.
3. `$execute`에서 `$qa`로 넘어가는 handoff가 너무 크거나 작지 않은지 다듬는다.
4. `$qa` reviewer lane이 code/harness 변경을 잘 나누는지 실제 변경으로 확인한다.
5. hidden support가 메인 UX를 가리지 않게 runtime 문서를 계속 얇게 유지한다.
